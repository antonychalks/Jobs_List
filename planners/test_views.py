...
class TestPlannerViews(TestCase):
    ...
    def setUp(self):
        """
        Sets up the test case with a test user
        and profile, as well as a test job.
        """
    ...
    def test_planner_home_view(self):
        """
        Tests the planner home view renders
    ...
    def test_user_list_view(self):
        """
        Tests the user list view renders
        correctly and with the correct template.
        """
        self.client.login(username='testUser', password='password')
        ...
        self.assertTemplateUsed(
            response,
            'planners/list_user.html'
        )

    def test_job_edit_view(self):
        """
        Tests the job edit view changes the details
        of the job and the user is redirected.
        """
        self.client.login(username='testUser', password='password')
        job = Job.objects.get(pk=self.job.id)
        response = self.client.post(reverse(
            'job_edit',
            args=[job.slug, job.id]), {
            'customer_name': "Test Customer",
            'email': "test@test.com",
            'phone': "07123456789",
            'other_phone': "",
    ...
    def test_user_detail_view(self):
        """
        Tests the user details view renders correctly,
        with the correct template and the correct user is loaded.
        """
        self.client.login(username='testUser', password='password')
    ...
    def test_add_user_view(self):
        """
        Tests the add user view renders
        correctly and with the correct template.
        """
        self.client.login(username='testUser', password='password')
