...
class TestTradesmanForm(TestCase):
    ...
    def setUp(self):
        """
        Sets up the test case by creating a test
        user with the role of 1 (tradesman),
        ...
        self.tradesmanProfile = (
            UserProfile.objects.get(user=self.tradesmanUser)
        )
    ...
    def test_add_task_form_invalid(self):
        """
        Tests the add_task_form returns invalid
        when the trades_required field has the wrong format.
        """
        add_task_form = AddTaskForm({
            'description': 'Example description',
    ...
    def test_edit_task_form_invalid(self):
        """
        Tests the edit_task_form returns invalid when
        the trades_required field has the wrong format.
        """
        form_data = {
            'description': 'Example description',
            'trades_required': ['planner'],
    ...
    def test_assign_tradesman_form_valid(self):
        ...
        self.assertIn(
            self.tradesmanProfile,
            self.task.tradesman_assigned.all()
        )

    def test_assign_tradesman_form_invalid(self):
        """
        Tests the assign_tradesman_form returns invalid
        when the tradesman_assigned field is passed a string
    ...
    def test_update_contact_details_form_invalid(self):
        """
        Tests the update_contact_details_form
        returns invalid when the form is invalid.
        """
        form_data = {
            'customer_name': 'John Doe',
            'phone': '0712547854',
